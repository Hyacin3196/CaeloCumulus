...
func _physics_process(delta):
	var position3D = _player.get_mouse()
	self.rotation = Vector3(0, atan2(
		  self.global_transform.origin.x -position3D.x, 
		 self.global_transform.origin.z -position3D.z 
...
func _aim_shoot():
	if(bullet_delay <= 0):
		...
		new_bullet.rotation.y =  _player.rotation.y + self.rotation.y
		new_bullet.velocity = Vector3(
			-(self.global_transform.basis.z.x) * BULLET_SPEED,
			0,
			-(self.global_transform.basis.z.z)* BULLET_SPEED)
